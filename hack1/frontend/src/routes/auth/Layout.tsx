...
const AuthLayout = () => {
  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (location.pathname === '/login') {
      ...
      if (password !== confirmPassword) { // Assuming `confirmPassword` is the state for the "Confirm Password" input.
        toast({
          description: "Passwords do not match",
          variant: 'destructive', // Assuming 'destructive' is a valid variant for an error message.
        });
        return; // Return early so that we don't proceed with the registration.
      }
